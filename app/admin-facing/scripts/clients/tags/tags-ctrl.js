 ...
 */
    .controller('ClientTagsController', function ($scope, focus, $filter, Tag, TeamTag, Client, $q, $route) {
        ...
        $scope.showPopover = function () {
            $scope.teamConflictWarning.$promise.then($scope.teamConflictWarning.show);
        };
        ...
        $scope.checkForConflicts = function (isValid) {
            Tag.checkForConflicts(Client.getClient(), $scope.removedTags).then(function (conflictingTeamTags) {
                if (conflictingTeamTags.length > 0) {
                    if (conflictingTeamTags.length === 1 && (conflictingTeamTags[0].conflictingTeamIds[0] === parseInt($route.current.params.team))) {
        ...
        return {
            ...
            link: function (scope, element) {
                element.on('click', function (event) {
                    ...
                    scope.$on('teamConflict.show.before', function () {
